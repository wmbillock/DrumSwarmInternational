...
class TestDryRunToolExecutor:
    def _make_executor(self):
        ...
        registry.register("create_coordinate", lambda **kw: kw)
        registry.register("submit_work", lambda **kw: kw)
        return DryRunToolExecutor(registry)

    def test_records_calls(self):
        executor = self._make_executor()
        result = executor.execute(None, "session-1", "create_coordinate", {"type": "movement", "title": "test"})
        assert result.success
        assert len(executor.recorded_calls) == 1
        assert executor.recorded_calls[0].tool_name == "create_coordinate"

    def test_synthetic_result(self):
        executor = self._make_executor()
        result = executor.execute(None, "s", "create_coordinate", {"type": "set", "title": "t"})
    ...
    def test_summary(self):
        ...
        executor.execute(None, "s", "create_coordinate", {"type": "set", "title": "t"})
    ...
    def test_does_not_check_permissions(self):
        ...
        result = executor.execute(None, "nonexistent", "create_coordinate", {"type": "movement", "title": "t"})
        assert result.success
