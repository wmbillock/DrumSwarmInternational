...
The trophy showcase will be displayed in a new "Results and Rankings" tab with separate trophy and standings subtabs:
...
5. **Tab location confirmed**: Trophies display in new "Results and Rankings" tab (relocated from "Swarm Health")
6. **Separate subtabs required**: Trophy and standings sections must be distinct subtabs within "Results and Rankings" tab
7. **Multiple trigger points**: Achievement checks must be triggered during rehearsals (for exceptional events) and after competitions/finals (for formal recognition)
...
1. **Define achievement detection trigger points**
   - Identify exceptional event triggers for rehearsal-time awards
   - Define post-competition and post-finals award moments
   - Specify which lifecycle hooks will invoke achievement detection

2. **Wire achievement detection into agent session lifecycle**
   - Call `/awards` endpoint at defined trigger points (rehearsal completion, post-competition, post-finals)
   - Persist returned awards to database
   - Ensure non-blocking execution (achievement detection runs asynchronously)

3. **Create "Results and Rankings" tab structure**
...
4. **Implement trophy subtab display**
...
5. **Implement standings subtab display**
...
- **Trigger points**: Achievement checks will be wired into rehearsal wrap-up, post-competition performance completion, and post-finals moments
...
- [ ] Identify exact lifecycle hooks for achievement detection (rehearsal completion, post-competition, post-finals)
- [ ] Define exceptional event criteria for rehearsal-time trophy awards
- [ ] Wire `/awards` endpoint calls into agent session completion handlers at all trigger points
- [ ] Implement async/non-blocking award persistence to database
- [ ] Create "Results and Rankings" tab UI structure with routing
- [ ] Implement trophy subtab with `TrophyShowcase` component integration
- [ ] Implement standings subtab with competition standings/rankings display
- [ ] Query `/awards/summary` in frontend and bind results to `TrophyShowcase` component
